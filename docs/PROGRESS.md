...
- **Spec 04 — Ingestion Pipeline** (`docs/specs/04-ingestion-pipeline.md`, still Draft). Unblocked by 02 + 03. Wires Rails `Internal::IngestController` + HMAC verify + Channel registration lookup + `Ingestion::ProcessMessage` service + `Contacts::Resolve` / `Conversations::ResolveOrCreate` / `Messages::Create`. Also adds the first channel container (`packages/channels/whatsapp-cloud`) using the gem, plus `infra/dev-webhook` and LocalStack in `docker-compose.yml`. End-to-end test: real webhook → dev-webhook → local queue → container → /internal/ingest → DB → Turbo Stream broadcast.
- **Spec 05 — Outbound Dispatch** (`docs/specs/05-outbound-dispatch.md`, still Draft). Also unblocked by 02 + 03. Adds `DispatchClient` path dep into Rails `Gemfile`, wires `SendMessageJob` (Solid Queue) + reply form.
