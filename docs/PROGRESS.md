...
| 05 | [Outbound Dispatch](./specs/05-outbound-dispatch.md)                                    | In Progress | 05a, 05b, 05c, 05d  | Depends on 02 + 03 + 04. Split into 4 plans (service/job, container send, reply form, status display). |
...
- **Spec 05 — Outbound Dispatch** (`docs/specs/05-outbound-dispatch.md`, still Draft). Unblocked by 02 + 03 + 04. Adds `DispatchClient` path dep into Rails `Gemfile`, wires `SendMessageJob` (Solid Queue) + reply form.
- **Spec 06 — Assignment, Transfer & Agent Workspace** (`docs/specs/06-assignment-transfer-workspace.md`, still Draft). Depends on 04 + 05.
